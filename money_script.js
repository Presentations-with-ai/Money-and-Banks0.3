...
const AGREEMENT_TEXT_HTML = `
      ...
      <p class="agreement-doc-meta">Вступление в силу: после нажатия "Принять"<br>Telegram-бот: Money and Banks Quiz<br>Владелец бота: Sayfiddinov™<br>Контакты: @SayfiddinovM</p>
      <h4 class="agreement-doc-section">1. Общие положения</h4>
      <p>1.1. Настоящее Пользовательское соглашение (далее - «Соглашение») регулирует порядок использования Telegram-бота Money and Banks Quiz (далее - «Бот»), содержащего тесты, задания, обучающие материалы, интерфейсы прохождения тестирования, подсчет результатов, статистику, а также иные связанные сервисы и функции.</p>
      ...
      <p>2.1. Бот - Telegram-бот Money and Banks Quiz, включая все диалоги, команды, встроенные кнопки, сообщения, сценарии, программный код, базы данных, тесты, результаты, визуальные элементы, тексты, изображения, скрипты и иные компоненты.</p>
...
function enforceAgreementOnEntry() {
  ...
  if (isTestPage) {
    window.location.replace('money_index.html');
    return false;
  }
...
<div class="card">
  ...
  document.getElementById('startBtn').onclick = () => {
    ...
    window.location.href = 'money_test.html';
  };
}
...
function startTest() {
  ...
  const themeFile = localStorage.getItem(THEME_FILE_KEY) || 'money_tests.json';
...
function finish() {
  ...
  card.innerHTML = `
    ...
    <button class="main" onclick="window.location.href='money_index.html'">🏠 В главное меню</button>
  `;
}
