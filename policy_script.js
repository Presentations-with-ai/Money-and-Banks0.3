...
function enforceAgreementOnEntry() {
  ...
  if (isTestPage) {
    window.location.replace('policy_index.html');
    return false;
  }
...
<div class="card">
  ...
  document.getElementById('startBtn').onclick = () => {
    ...
    window.location.href = 'policy_test.html';
  };
}
...
function startTest() {
  ...
  fetch(themeFile)
    ...
    .catch(err => {
      ...
      window.location.href = 'policy_index.html';
    });
}
...
function finish() {
  ...
  card.innerHTML = `
    ...
    <button class="main" onclick="window.location.href='policy_index.html'">🏠 В главное меню</button>
  `;
}
