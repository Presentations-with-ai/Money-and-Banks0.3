...
function enforceAgreementOnEntry() {
  ...
  if (isTestPage) {
    window.location.replace('macro_index.html');
    return false;
  }
...
<div class="card">
  ...
  document.getElementById('startBtn').onclick = () => {
    ...
    window.location.href = 'macro_test.html';
  };
}
...
function startTest() {
  ...
  const themeFile = localStorage.getItem(THEME_FILE_KEY) || 'macro_tests.json';
  ...
  fetch(themeFile)
    ...
    .catch(err => {
      ...
      window.location.href = 'macro_index.html';
    });
}
...
function finish() {
  ...
  card.innerHTML = `
    ...
    <button class="main" onclick="window.location.href='macro_index.html'">🏠 В главное меню</button>
  `;
}
